...
function Event({ user, token, allEvents, setAllEvents }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setAllEvents(data); //[use, setUse] = useState(wich start?)
      } catch (error) {
        console.log(error);
      }
    };
  ...
  const publicEvents = allEvents.filter((event) => {
    return event.radioEvent == "Public";
  });
                  ...
                  <div className="flex flex-row items-center">
                    ...
                    >
                      {user._id in event.likes ? (
                        <AiFillHeart onClick={(e) => handleLike(e, event)} />
                      ) : (
                        <AiOutlineHeart onClick={(e) => handleLike(e, event)} />
                      )}
                    </button>
              ...
              <form method="dialog" className="modal-box max-w-[75vw]">
                ...
                <div className="flex flex-row">
                  <div className="max-h-[32rem]">
                    <img className="h-full" src={event.picturePath} alt="" />
                  </div>
                    ...
                    <div className="flex flex-row gap-x-4 gap-y-4 py-4 flex-wrap">
                      ...
                      <div className="flex flex-row items-center">
                        ...
                        <span className="teko-text text-2xl">
                          Genre: {event.genre}
                          <span className="font-bold">Techno/Alternative</span>
                        </span>
                      </div>
                      <div className="flex flex-row items-center">
                        ...
                        <span className="teko-text text-2xl">
                          Time:{" "}
                          <span className="font-bold">{event.time} Uhr</span>
                        </span>
                      </div>
                      <div className="flex flex-row items-center">
